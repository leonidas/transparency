(function() {
  var matchingElements, renderChildren, renderDirectives, renderNode, renderSimple, renderValues, templates;

  templates = [];

  jQuery.fn.render = function(objects, directives) {
    var context, contexts, n, object, t, template, _i, _j, _len, _len2;
    contexts = this;
    if (!(objects instanceof Array)) objects = [objects];
    directives || (directives = {});
    for (_i = 0, _len = contexts.length; _i < _len; _i++) {
      context = contexts[_i];
      if (context.transparencyId == null) {
        context.transparencyId = templates.length;
      }
      t = context.cloneNode(true);
      if (!templates[context.transparencyId]) templates.push(t);
  ...
  renderChildren = function(template, object, directives) {
    ...
    for (key in object) {
      value = object[key];
      if (typeof value === 'object') {
        _results.push(jQuery(matchingElements(template, key)).render(value, directives[key], key));
      }
    }
    return _results;
  };
