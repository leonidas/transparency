(function() {
  var TEXT_NODE, matchingElements, render, renderChildren, renderDirectives, renderValues, setValue;
  ...
  if (window.t == null) window.t = {};

  window.t.render = render = function(contexts, objects, directives) {
    var c, i, n, object, parent, sibling, template, _base, _base2, _i, _j, _k, _len, _len2, _len3, _ref, _ref2;
    ...
    for (_i = 0, _len = contexts.length; _i < _len; _i++) {
      ...
      (_base2 = c.t).template || (_base2.template = ((function() {
        var _results;
        _results = [];
      ...
      while (c.t.instances.length < objects.length) {
        c.t.instances.push(map((function(n) {
  ...
  renderChildren = function(template, object, directives) {
    ...
    for (k in object) {
      v = object[k];
      if (typeof v === 'object') {
        _results.push(render(matchingElements(template, k), v, directives[k]));
      }
    }
    return _results;
  };
  ...
  matchingElements = function(template, key) {
    ...
    return (_base2 = fc.t.qc)[key] || (_base2[key] = template.querySelectorAll("#" + key + ", " + key + ", ." + key + ", [data-bind='" + key + "']"));
  };
