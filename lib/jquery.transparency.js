  ...
  renderValues = function(template, object) {
    ...
    for (key in object) {
      ...
      if (typeof value === 'string') {
        if (template.hasClass(key || template.is(key))) {
          renderNode(template, value);
        }
        _results.push((function() {
          var _i, _len, _ref, _results2;
          _ref = template.find("" + key + ", ." + key);
          _results2 = [];
    ...
    for (key in object) {
      ...
      if (typeof value === 'object' && key !== 'parent_') {
        ...
        if (template.hasClass(key)) {
          template.render(value, directives[key], object);
        }
        template.find("." + key).data('key', key);
        _results.push(template.find("." + key).render(value, directives[key], object));
      }
    }
    return _results;
  };
