(function() {
  ...
  renderValues = function(template, object) {
    ...
    if (typeof object === 'object') {
      ...
      return setText(matchingElements(template, 'listElement')[0] || template.getElementsByTagName('*')[0], object);
    }
  };
  ...
  setText = function(e, text) {
    var t, textNode, _i, _len, _ref, _ref2;
    ...
    if (e.firstChild) {
      return e.insertBefore(textNode, e.firstChild);
    } else {
      return e.appendChild(textNode);
    }
  };
  ...
  elementMatcher = function(key) {
    return function(element) {
      return element.className.split(' ').indexOf(key) > -1 || element.id === key || element.nodeName.toLowerCase() === key.toLowerCase() || element.getAttribute('data-bind') === key;
    };
  };
