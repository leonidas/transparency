...
(function() {
  var ELEMENT_NODE, T, TEXT_NODE, Transparency, attr, cloneNode, debug, debugMode, elementMatcher, elementNodes, expando, html5Clone, isDate, isPlainValue, matchingElements, prepareContext, renderChildren, renderDirectives, renderValues, setContent, setHtml, setText, _base,
    __slice = [].slice;
  ...
  attr = function(element, attribute, value) {
    ...
    switch (attribute) {
      case 'text':
        (_base = elementData.attributes)['text'] || (_base['text'] = element.textContent || element.innerText);
        if (value) {
